	...
	class IncludePageTag < Jekyll::Tags::IncludeTag
		...
		def render(context)
			...
			content_with_include_parameters = context.stack do
				...
				# Renders to inclusion's private var @template
				inclusion.render(context)
			end

			# Construct a new page so we can render it
			dir = File.join(page_path(context), File.dirname(inclusion.name))
			name = File.basename(inclusion.name)
			newpage = Jekyll::Page.new(@site, @site.source, dir, name)
			newpage.content = content_with_include_parameters # Set content to result of inclusion
